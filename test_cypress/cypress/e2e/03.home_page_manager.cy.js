...
const test_project_name_suffix = "_HOMEPAGE_MANAGER_AS_OWNER";
...
it("should by click to display a new bot creation modal - H2", () => {
...
it("should by click to display a modal to check detailed usage - H10.", () => {
  cy.visit(base_url, {
    onBeforeLoad(win) {
      win.localStorage.setItem('onboarding', 'false');
    },
...
it("should by list of projects I am in charge of - H11.", () => {
...
it("should by click to go to chat - H12., H13.Click to go to bot details, H14.Click to show delete modal, H15.Click to clone the project. Project names and descriptions are duplicated, but files and items are not., H17.Click to go to chat, H18.Click to go to bot details, H21.Click to go to chat, H22.Click to go to bot details", () => {
...
it("should by click to go to the top of the bot list - H26.", () => {
  let bProcessed = false;
  cy.get(container_project_list).contains(group_list_title).parent().children().each(($child, index, $list) => {
    if (index === 0) return;
    if (bProcessed) return;
