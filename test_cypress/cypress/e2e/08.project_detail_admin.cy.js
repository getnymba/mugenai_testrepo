...
const test_project_name = "Cypress test " + Math.floor(Date.now() / 1000) + "_PROJECT_DETAIL_ADMIN_AS_OWNER";
  ...
  project_list_group.children().each(($child, index, $list) => {
    ...
    if ($child[0].innerText.startsWith(test_project_name)) {
      ...
      if ($list["length"] - 1 === index) {
        ...
        const modal_create_project_systemprompt = `${modal_create_project} > :nth-last-child(3).v-col .v-field__input`;
...
it("P1.Displays the project name, P2.Display project description, P3.system prompt is displayed, P4.A feature like project owners can share prompts.Press into the prompt area to copy, P7.Edit project name, description and system prompt, P8.transition to chat", () => {
  cy.wrap(
  ...
  project_list_group.children().each(($child, index, $list) => {
    ...
    if ($child[0].innerText.startsWith(test_project_name)) {
      ...
      cy.get('.v-alert').should('have.text', test_project_name + " system prompt");
      ...
      cy.get(system_prompt_container).find(system_prompt).each(($child, index, $list) => {
        cy.wrap($child).click({ force: true });
        cy.window().then((win) => {
          win.navigator.clipboard.readText().then((text) => {
            expect(text).to.eq(test_project_name + " system prompt");
          });
        });
      });
      ...
      // P7 Edit project button  You can edit the project name, description, system prompts, and the model used for the API.
// You can upload project icons
      let project_buttons_container = ".v-container > :nth-child(1).v-row > .v-col-sm-12 > button";
      cy.get(project_buttons_container).contains("settings").click({ force: true });
      cy.get('.v-toolbar__content > .v-toolbar-title > .v-toolbar-title__placeholder').should('have.text',"Edit Project");
      const selector_title = ".v-container > .v-form > .v-row > :nth-child(1).v-col > .v-input > .v-input__control > .v-field > .v-field__field > .v-field__input > input";
      const selector_desc = ".v-container > .v-form > .v-row > :nth-child(2).v-col > .v-input > .v-input__control > .v-field > .v-field__field > .v-field__input";
      const selector_prompt = ".v-container > .v-form > .v-row > :nth-child(3).v-col > .v-input > .v-input__control > .v-field > .v-field__field > .v-field__input";
      const selector_icon = ".v-container > .v-form > .v-row > :nth-child(4).v-col > .v-input > .v-input__control > .v-field";
      const selector_model = ".v-container > .v-form > .v-row > :nth-child(5).v-col > .v-input > .v-input__control > .v-field";
      const selector_submit = ".v-container > .v-form > .v-row > :nth-last-child(1).v-col > button";
      // cy.get('.v-overlay').find('.v-card > .v-container > .v-row > :nth-child(2) > .v-btn').click({ force: true });
      cy.get(selector_title).type("_edited", { force: true });
      cy.get(selector_desc).type("_edited", { force: true });
      cy.get(selector_prompt).type("_edited", { force: true });
      cy.get(selector_icon).click({ force: true });
      cy.get('input[type="file"]').attachFile('DUMMYPIC.png');
      cy.get(selector_model).click({ force: true });
      const model_list_options = ".v-list >:nth-child(3).v-list-item";
      cy.get(model_list_options).click({ force: true });
      cy.get(selector_model).should("not.contain", "GPT 3.5 Turbo");


      cy.get(selector_submit).click({ force: true });
      cy.wait(10000);
