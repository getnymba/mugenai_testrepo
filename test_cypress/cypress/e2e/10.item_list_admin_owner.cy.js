...
const test_project_name_suffix = " item list by ADMIN_OWNER";
...
project_list_group.children().each(($child, index, $list) => {
    ...
    if ($list["length"] - 1 === index) {
        ...
        if ($child[0].innerText.startsWith(test_project_name)) {
            ...
            let group_title_item = cy.get(".v-container > .v-row > .v-col > .v-card > .v-row > .v-col").contains("Item List");
...
describe('Item list field function check by ADMIN role, project OWNER',() => {

    it("I1 - The item name, item description, and creation date are displayed. I13. Show tag list", () => {
        let bProcessed = false;
        const container_project_list = ".v-container > :nth-child(2) > .v-row > .v-col";
        ...
        project_list_group.children().each(($child, index, $list) => {
            ...
            cy.get(".v-row > .v-col > .v-card > .v-row").contains("Item List").wait(wait_time)
            .parent().next().find(item_path).then((next) => {
                
                cy.wrap(next).eq(1).should('contain',item_name);
                cy.wrap(next).eq(2).should('contain',item_dis);
    
                // check  date
                cy.wrap(next).eq(3).invoke('text').then((dateText) => {
                    cy.log(dateText)
                });
            });
            //I13 Show tag list 
            cy.get(".v-row > .v-col > .v-card > .v-row").contains("Tag Variable List").wait(500).parent().next().children().find("table").should('exist');
            }
    ...
    it("I2 - The item name, item description, creation date, and action buttons are displayed.", () => {
        let bProcessed = false;
        const container_project_list = ".v-container > :nth-child(2) > .v-row > .v-col";
        let project_list_group = cy.get(container_project_list).contains(group_list_title).parent();
        project_list_group.children().each(($child, index, $list) => {
            ...
            .parent().next().find(item_path).then((next) => {
                ...
                // check  date
                cy.wrap(next).eq(4).invoke('text').then((dateText) => {
                    cy.log(dateText)
                });
    
                // check edit and delete buttons
                cy.wrap(next).eq(5).should('contain','edit')
                cy.wrap(next).eq(5).should('contain','delete')
            });
    ...
    it("I4 - Display the item image.", () => {
        ...
        project_list_group.children().each(($child, index, $list) => {
            ...
            let group_title_item = cy.wait(wait_time).get(".v-row").contains("Item List");
            let next = group_title_item.parent().next();
            const item_path = ".v-table > .v-table__wrapper > table > tbody > tr:nth-child(1) > td.py-2 > div > div > div > div > div > div.v-responsive__content > div > div.v-responsive.v-img > img"
    ...
    it("I5, I6 - Edit the item name and item description. Delete the item.", () => {
        let bProcessed = false;
        const container_project_list = ".v-container > :nth-child(2) > .v-row > .v-col";
        let project_list_group = cy.get(container_project_list).contains(group_list_title).parent();
        project_list_group.children().each(($child, index, $list) => {
            ...
            .parent().next().then((next) => {
                const item_path = ".v-table > .v-table__wrapper > table > tbody > tr > td";
                cy.wrap(next).find(item_path).contains(item_name).nextAll().eq(3).then((edit_button_path) =>{
                    cy.wrap(edit_button_path).wait(wait_time*2).contains('edit').click({ force: true });
                })
            ...
            //delete item
            cy.wait(wait_time).get(".v-row > .v-col > .v-card > .v-row").contains("Item List").wait(wait_time)
            .parent().next().then((next) => {
                const item_path = ".v-table > .v-table__wrapper > table > tbody > tr > td";
                cy.wrap(next).find(item_path).contains('item 2').nextAll().eq(3).then((delete_button_path) => {
                    cy.wrap(delete_button_path).wait(wait_time*2).contains('delete').click({ force: true });
                    cy.get(".v-overlay > .v-overlay__content > .v-sheet > .v-btn").click({ force: true })
                })
            });
    ...
    it("I7, I8, I9 - Upload an image from local storage. Enlarge and Delete the uploaded image.", () => {
        let bProcessed = false;
        const container_project_list = ".v-container > :nth-child(2) > .v-row > .v-col";
        let project_list_group = cy.get(container_project_list).contains(group_list_title).parent();
        project_list_group.children().each(($child, index, $list) => {
            ...
            cy.wait(wait_time*2);
            cy.get('.v-btn.v-btn--block.v-btn--flat.v-theme--darkTheme.bg-primary.v-btn--density-comfortable.v-btn--size-default.v-btn--variant-elevated.small-text').click();
            cy.wait(wait_time*4);
            cy.reload(true);
            ...
            .parent().next().then((next) => {
                const item_path = ".v-table > .v-table__wrapper > table > tbody > tr > td";
                cy.wrap(next).find(item_path).contains(item_name).prevAll().eq(0).then((add_iamge_button_path) => {
                    cy.wrap(add_iamge_button_path).wait(wait_time*2).contains('Add Image').click({ force: true });
                })
            });
    
            cy.get('input[type="file"]').attachFile('DUMMYPIC.png');
            cy.get('.v-overlay').find('.v-card > .v-container > .v-row > :nth-child(2) > .v-btn').click({ force: true });
            //cy.get('.v-overlay__scrim').contains('button', 'Add Image').click({ force: true });
            cy.wait(wait_time*2);
            cy.reload(true).wait(wait_time);
        
            cy.wait(wait_time/2).get(".v-row > .v-col > .v-card > .v-row").contains("Item List").wait(wait_time)
            .parent().next().then((next) => {
                const item_path = ".v-table > .v-table__wrapper > table > tbody > tr > td";
                cy.get(next).find(item_path).contains(item_name).prev().wait(wait_time*2).then((items) => {
    
                    //zoom image
                    cy.wrap(items).contains('zoom_in').click();
                    const close_buttom_path = ".v-overlay-container > .v-overlay > .v-overlay__content > .v-card > .v-toolbar > .v-toolbar__content"
    ...
    it("I10 - Upload a CSV file from local storage that contains two columns-*title* for the item name, and *description* for the item description, with the character encoding set as UTF-8.", () => {
        let bProcessed = false;
        const container_project_list = ".v-container > :nth-child(2) > .v-row > .v-col";
        let project_list_group = cy.get(container_project_list).contains(group_list_title).parent();
        project_list_group.children().each(($child, index, $list) => {
            ...
            let group_title_item = cy.wait(wait_time/2).get(".v-row > .v-col > .v-card > .v-row").contains("Item List");
            let next = group_title_item.parent();
    ...
    it("I11 - Enter the item name and item description to add an item. Uploading an image is optional.", () => {
        let bProcessed = false;
        const container_project_list = ".v-container > :nth-child(2) > .v-row > .v-col";
        let project_list_group = cy.get(container_project_list).contains(group_list_title).parent();
        project_list_group.children().each(($child, index, $list) => {
    ...
    it("I12 - Go to the previous page, go to the next page, move to the designated page. One page includes ten files.", () => {
        let bProcessed = false;
        const container_project_list = ".v-container > :nth-child(2) > .v-row > .v-col";
        let project_list_group = cy.get(container_project_list).contains(group_list_title).parent();
        project_list_group.children().each(($child, index, $list) => {
            ...
            let group_title = cy.get(".v-row > .v-col > .v-card > .v-row").contains("Item List");
            let next = group_title.parent().next();
            const table_body = ".v-table > .v-table__wrapper > table > tbody";
            next.find(table_body).children().each(($child, index, $list) => {
                cy.expect($list["length"]).to.be.lessThan(11);
                const button_path = ".v-row > .v-col > .v-table > .v-row > .v-col > .d-flex";
                cy.get(button_path).contains('chevron_right').then(($nav) => {
                    if ($nav[0].getAttribute("disabled") == null) {
                        cy.wrap($nav[0]).click({ force: true });
                    }
                })
                
                cy.get(".v-row > .v-col > .v-table > .v-row > .v-col > .d-flex > .v-pagination > .v-pagination__list > .v-pagination__item").contains("1").click({ force: true });
            })
            }
        });
    ...
    const create_item = () => {
        //Creating new item
        let group_title_item = cy.get(".v-row > .v-col > .v-card > .v-row > .v-col").contains("Item List");
        let next = group_title_item.parent();
        next.find('button').contains('Add Item').click();
        cy.get('input[placeholder="Item Name"]').type(item_name);
        cy.get('textarea.v-field__input').type(item_dis);
