...
const test_project_name_suffix = "_DATA_ACQUISITION_ADMIN_AS_OWNER";
  ...
  project_list_group.children().each(($child, index, $list) => {
    ...
    if ($child[0].innerText.startsWith(test_project_name)) {
      ...
      if ($list["length"] - 1 === index) {
        ...
        const modal_create_project_btnsubmit = `${modal_create_project} > :nth-last-child(1).v-col button`;
...
it("D1.You can upload a file from local by pressing the file selection button. Only pptx, pdf, excel, csv, word, txt formats can be uploaded.", () => {
  openProject(() => {
...
it("D4.File name, URL, process type, status, file size, chunk count, update date, and action are displayed.Only actions are not visible to project general users., D6.Show status as completed, pending and failed", () => {
  openProject(() => {
    const project_title = ".v-container > .v-row > :nth-child(1)";
    const project_title_element = cy.get(project_title);
    project_title_element.should('contain.text', test_project_name);
...
// it("D8.There is a dropdown in Pagination where you can choose from 10, 25, 50, 100, all. There are buttons to transition to the next page and the previous page.", () => {
//   openProject(() => {
//     const project_title = ".v-container > .v-row > :nth-child(1)";
//     const project_title_element = cy.get(project_title);
//     project_title_element.should('contain.text', test_project_name);
...
//     const footer_info = ".v-table > .v-row";
...
//       button_active_next = false;
//       button_active_prev = false;
...
//         data_acquisition_title = cy.get(".v-row").contains("Data Acquisition Process List");
//         next = data_acquisition_title.parent().next();
//         const footer_buttons = ".v-table > .v-row > .v-col > .d-flex > .v-pagination";
//         // Click on "next page"
...
//           total_elements = parseInt(array_info3[1].trim(), 10);
//           let elements_from_2 = parseInt(array_info2[0].trim(), 10);
//           let elements_to_2 = parseInt(array_info3[0].trim(), 10);
//           cy.log("total_elements: " + total_elements);
//           cy.log("From: " + elements_from + ", To: " + elements_to);
//           button_active_next = false;
//           button_active_prev = false;
...
//               const array_info3 = array_info2[1].split(" of ");
//               total_elements = parseInt(array_info3[1].trim(), 10);
//               let elements_from_3 = parseInt(array_info2[0].trim(), 10);
//               let elements_to_3 = parseInt(array_info3[0].trim(), 10);
//               cy.log("total_elements: " + total_elements);
//               cy.log("From: " + elements_from + ", To: " + elements_to);
//               button_active_next = false;
//               button_active_prev = false;
...
//                   const array_info3 = array_info2[1].split(" of ");
//                   total_elements = parseInt(array_info3[1].trim(), 10);
//                   let elements_from_4 = parseInt(array_info2[0].trim(), 10);
//                   let elements_to_4 = parseInt(array_info3[0].trim(), 10);
//                   cy.log("total_elements: " + total_elements);
//                   cy.log("From: " + elements_from + ", To: " + elements_to);
//                   button_active_next = false;
//                   button_active_prev = false;
...
//                       const array_info3 = array_info2[1].split(" of ");
//                       total_elements = parseInt(array_info3[1].trim(), 10);
//                       let elements_from_5 = parseInt(array_info2[0].trim(), 10);
//                       let elements_to_5 = parseInt(array_info3[0].trim(), 10);
//                       cy.log("total_elements: " + total_elements);
//                       cy.log("From: " + elements_from + ", To: " + elements_to);
//                       button_active_next = false;
//                       button_active_prev = false;
...
//     // 25
//     const footer_items_per_page = ".v-table > .v-row > .v-col > .d-flex > .v-input > .v-input__control > ";
//     data_acquisition_title = cy.get(".v-row").contains("Data Acquisition Process List");
//     next = data_acquisition_title.parent().next();
...
//       const array_info3 = array_info2[1].split(" of ");
//       total_elements = parseInt(array_info3[1].trim(), 10);
//       let elements_from = parseInt(array_info2[0].trim(), 10);
//       let elements_to = parseInt(array_info3[0].trim(), 10);
//       cy.log("total_elements: " + total_elements);
//       cy.log("From: " + elements_from + ", To: " + elements_to);
