...
let temp_guest_url = Cypress.env('temp_guest_url_home');
let temp_guest_password = Cypress.env('temp_guest_password_home');
...
const test_project_name_suffix = "_HOMEPAGE_TEMPGUEST_AS_NOT_OWNER";
...
it("should by click to go to home page - H1", () => {
  cy.get('.app-bar-logo').click();
  cy.url().should('eq', `${base_url}/`);
});

it("should by click to reveal the Notifications dropdown. On the other hand, when the 'see all' button is pressed, the screen moves to the 'notice list' screen - H3", () => {
...
it("should by click to show logout to non-admin users - H5", () => {
  const user_icon = ".v-toolbar > .v-toolbar__content > .v-container > :nth-child(6).v-btn > .v-btn__content > i";
  cy.get(user_icon).should('have.text', 'account_circle');
  cy.get(user_icon).contains("account_circle").click({ force: true });
  cy.wait(2000);
...
it("should By clicking on the language switch, you can choose between two options, Japanese and English - H8", () => {
  const language_container = ".v-toolbar > .v-toolbar__content > .v-container";
  cy.get(language_container).contains("language").click({ force: true });

  cy.get(".v-overlay .v-list > :nth-child(2).v-list-item").click({ force: true });

  let project_title = ".v-container > .v-row > :nth-child(1)";
  let project_title_element = cy.get(project_title);
  let project_buttons = project_title_element.next();
  project_buttons.contains("設定").should("exist");

  cy.get(language_container).contains("language").click({ force: true });

  cy.get(".v-overlay .v-list > :nth-child(1).v-list-item").click({ force: true });

  project_title = ".v-container > .v-row > :nth-child(1)";
  project_title_element = cy.get(project_title);
  project_buttons = project_title_element.next();
  project_buttons.contains("Settings").should("exist");
});
...
it("should click to display a modal to check detailed usage - H10", () => {
  cy.visit(base_url, {
    onBeforeLoad(win) {
      win.localStorage.setItem('onboarding', 'false')
    },
...
it("should invitation project - H16", () => {
  cy.visit(base_url, {
    onBeforeLoad(win) {
      win.localStorage.setItem('onboarding', 'false')
    },
...
it("should by click to go to chat, H18.Click to go to bot details - H17.", () => {
...
it("should Hide target project -  H23, H24.Hidden project list, H25.Click to display the target project. Restore the original list.", () => {
  cy.visit(base_url, {
    onBeforeLoad(win) {
      win.localStorage.setItem('onboarding', 'false')
    },
...
it("should by click to view bots in a grid or list - H27.", () => {
  cy.visit(base_url, {
    onBeforeLoad(win) {
      win.localStorage.setItem('onboarding', 'false')
    },
