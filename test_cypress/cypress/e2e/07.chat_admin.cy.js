...
const test_project_name = "Cypress test " + Math.floor(Date.now() / 1000) + "_CHAT_ADMIN_AS_OWNER";
  ...
  project_list_group.children().each(($child, index, $list) => {
    ...
    if ($child[0].innerText.startsWith(test_project_name)) {
      ...
      if ($list["length"] - 1 === index) {
        ...
        //system prompt aguulsan textbox olon bgagaas logictoi hailgaj cadaagui
        ...
        cy.get(modal_create_project).contains('button','Create Project').click({force: true})
...
it("should Show bot name, bot description is displayed, Displays a button that can display a system prompt, You will see the chat between the bot and the user, Click to enter a message to the bot, change the model, Generate a URL. A modal appears where you can copy the URL and password, A message is sent to the bot, Transition to project details C1-C8, C10", () => {
  openProject(() => {
    // C1 check bot name is displayd
    //frecuently wait
    cy.wait(4000)
    cy.get('.text-h6').should('have.text', test_project_name)

    // C2 check bot desciption is displayd
    ...
    cy.get(".v-overlay > .v-overlay__content > .v-list").contains('GPT 4').click({ force: true });
    cy.get(".v-main > .v-container > .v-row > :nth-child(2).v-col-sm-12 > .v-input > .v-input__control > .v-field").should("contain.text", "GPT 4").click();
    cy.get(".v-overlay > .v-overlay__content > .v-list").contains('Palm 2').click({ force: true });
    cy.get(".v-main > .v-container > .v-row > :nth-child(2).v-col-sm-12 > .v-input > .v-input__control > .v-field").should("contain.text", "Palm 2").click();
    cy.get(".v-overlay > .v-overlay__content > .v-list").contains('GPT 3.5 Turbo').click({ force: true });
    ...
    // C7 URL creation
    cy.get('.v-main > .v-container > .v-row > :nth-child(2).v-col-sm-12').contains("link")//.click({ force: true });
