...
const input_email =
  ".v-form > .v-row > :nth-child(3) > .v-input > .v-input__control > .v-field > .v-field__field > .v-field__input input";
const input_password =
  ".v-form > .v-row > :nth-child(4) > .v-input > .v-input__control > .v-field > .v-field__field > .v-field__input input";
...
const current_email = Cypress.env("email_guest_v2");
const current_password = Cypress.env("password_guest_v2");
...
const test_project_name_suffix = "_NEW_USER_INVITE_LIST_USER_AS_OWNER";
        ...
        project_list_group.children().each(($child, index, $list) => {
          ...
          if ($child[0].innerText.startsWith(test_project_name)) {
            ...
            for (let counter = 0; counter < dummy_user_count; counter++) {
              ...
              const selector_modal_input_email =
                ".v-overlay > .v-overlay__content > .v-card > .v-container > .v-form > .v-row > :nth-child(1) input";
              cy.get(selector_modal_input_email).type(
                dummy_user_list[counter]
              );
...
it("should check if the email address is not entered or there is an error in the email format, an error message will be displayed.error contents:Please enter your email address.The email address format is incorrect. (N2)", () => {
  ...
  project_list_group.children().each(($child, index, $list) => {
    ...
    if ($child[0].innerText.startsWith(test_project_name)) {
      ...
      const selector_modal_input_email =
        ".v-overlay > .v-overlay__content > .v-card > .v-container > .v-form > .v-row > :nth-child(1) input";
      cy.get(selector_modal_input_email).clear({ force: true });

      const selector_modal_submit =
        ".v-overlay > .v-overlay__content > .v-card > .v-container > .v-form > .v-row > :nth-child(3) button";
      cy.get(selector_modal_submit).click({ force: true });

      const selector_modal_input_email_message =
        ".v-overlay > .v-overlay__content > .v-card > .v-container > .v-form > .v-row > :nth-child(1) > .v-input > .v-input__details > .v-messages > .v-messages__message";
      cy.get(selector_modal_input_email_message).should(
        "have.text",
        "This field is required"
      );

      cy.get(selector_modal_input_email).type("username");
...
it("should show email address, username, role and action, N4.Display email address, username and role. (N3)", () => {
  let bProcessed = false;
  const container_project_list =
    ".v-container > :nth-child(2) > .v-row > .v-col";
  let project_list_group = cy
        ...
        .each(($child, index, $list) => {
          switch (index) {
            ...
            case 2:
              ...
              break;
            case 3:
              cy.wrap($child).find("div > span").should("have.text", "Actions");
              break;
            default:
              break;
          }
...
it("should move to the previous page/next page. 10, 25, 50, 100, all users are included per page. (N5)", () => {
  let bProcessed = false;
  const container_project_list =
    ".v-container > :nth-child(2) > .v-row > .v-col";
  let project_list_group = cy
  ...
  project_list_group.children().each(($child, index, $list) => {
    ...
    if ($child[0].innerText.startsWith(test_project_name)) {
      ...
      const footer_info =
        ".d-flex.justify-center.align-center small";

      next
        .find(".d-flex.justify-center.align-center small")
        .contains('of')
        .invoke("text")
        .then((text) => {
