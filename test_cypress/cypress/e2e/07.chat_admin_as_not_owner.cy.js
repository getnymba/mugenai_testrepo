...
const test_project_name = "Cypress test " + Math.floor(Date.now() / 1000) + "_CHAT_ADMIN_AS_NOT_OWNER";
      ...
      if ($list["length"] - 1 === index) {
        ...
        project_list_group.children().each(($child, index, $list) => {
          ...
          if ($child[0].innerText.startsWith(test_project_name)) {
            ...
            const selector_modal_submit = ".v-overlay > .v-overlay__content > .v-card > .v-container > .v-form > .v-row > :nth-child(3) > .v-btn";
...
it("should can Customize your chat. You can change the background color, input text area color, setting button color, and send button color. You can change the bot's avatar image and chat background image. There is also a button to make it the default - C9", () => {
  openProject(() => {
    // C9
    cy.get('.text-h6').should('have.text', test_project_name)
    cy.get(':nth-child(2) > .text-caption').should('have.text', test_project_name + " project description")
