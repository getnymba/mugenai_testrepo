...
const test_project_name_suffix = "_DATA_ACQUISITION_TEMP_GUEST_AS_NOT_OWNER";
        ...
        project_list_group.children().each(($child, index, $list) => {
          ...
          if ($child[0].innerText.startsWith(test_project_name)) {
            ...
            for (let counter = 0; counter < 25; counter++) {
              const data_acquisition_title = cy
                .get(".v-row")
                .contains("Data Acquisition Process List");
              data_acquisition_title
                .parent()
                .get("button")
                .contains("Data Acquisition")
                .click({ force: true });
              ...
              cy.get(
                ".v-overlay__content > .v-card > .v-container input[type='file']"
              ).attachFile(files[Math.floor(Math.random() * files.length)]);
              cy.get(".v-overlay__content > .v-card > .v-container")
...
it("should check file name, URL, process type, status, file size, chunk count, update date, and action are displayed.Only actions are not visible to project general users.(D4) Show status as completed, pending and failed. (D6)", () => {
  ...
  const col1 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(1).v-data-table__td span";
  const col2 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(2).v-data-table__td span";
  const col3 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(3).v-data-table__td span";
  const col4 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(4).v-data-table__td span";
  const col5 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(5).v-data-table__td span";
  const col6 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(6).v-data-table__td span";
  const col7 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(7).v-data-table__td span";
  const col8 =
    ".v-table > .v-table__wrapper > table > thead > tr > :nth-child(8).v-data-table__td span";
  next.find(col1).should("have.text", "File Name");

  data_acquisition_title = cy
    .get(".v-row")
    .contains("Data Acquisition Process List");
  ...
  data_acquisition_title = cy
    ...
    .contains("Data Acquisition Process List");
  next = data_acquisition_title.parent().next();
  next.find(col8).should("not.exist");

  const td_col1 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(1)";
  const td_col2 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(2)";
  const td_col3 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(3)";
  const td_col4 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(4)";
  const td_col5 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(5)";
  const td_col6 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(6)";
  const td_col7 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(7)";
  const td_col8 =
    ".v-table > .v-table__wrapper > table > tbody > tr > :nth-child(8)";
  ...
  next.find(td_col1).each(($el, index, $list) => {
    if ($el[0].innerText.startsWith("No data available")) {
      has_data = false;
    }
    cy.log("has_data: " + has_data);
    if (has_data) {
      cy.wrap($el)
        .invoke("text")
        .should(
          "satisfy",
          (text) =>
            text === "Lorem_ipsum.pdf" ||
            text === "derivative.pptx" ||
            text === "sampledatafootballplayers.xlsx" ||
            text === "dummy.csv" ||
            text === "8-2-book-review-6.pdf" ||
            text === "ps3_dev.pdf"
        );
      ...
      data_acquisition_title = cy
        ...
        .contains("Data Acquisition Process List");
      next = data_acquisition_title.parent().next();
      next.find(td_col8).should("not.exist");
    } else {
      cy.wrap($el)
        .invoke("text")
        .should("satisfy", (text) => text === "No data available");
    }
  });
...
it("should check if there is a dropdown in Pagination where you can choose from 10, 25, 50, 100, all. There are buttons to transition to the next page and the previous page. (D8)", () => {
  ...
  let next = data_acquisition_title.parent().next();
  const footer_info =
    ".v-table > .v-data-table-footer > .v-data-table-footer__info";
  ...
  next
    ...
    .then((text) => {
      ...
      next
        .find(table_body)
        .children()
        .should("have.length", elements_to - elements_from + 1);
        ...
        next
          ...
          .then((text) => {
            ...
            next
              .find(table_body)
              .children()
              .should("have.length", elements_to_2 - elements_from_2 + 1);
            if (button_active_next) {
              data_acquisition_title = cy
                .get(".v-row")
                .contains("Data Acquisition Process List");
              ...
              next
                ...
                .then((text) => {
                  ...
                  total_elements = parseInt(array_info[1].trim(), 10);
                  const array_current_page = array_info[0].trim().split("-");
                  let elements_from_3 = parseInt(
                    array_current_page[0].trim(),
                    10
                  );
                  let elements_to_3 = parseInt(
                    array_current_page[1].trim(),
                    10
                  );
                  cy.log("total_elements: " + total_elements);
                  cy.log(
                    "From3: " + elements_from_3 + ", To3: " + elements_to_3
                  );
                  let button_active_next = false;
                  let button_active_prev = false;
                  ...
                  next
                    .find(table_body)
                    .children()
                    .should("have.length", elements_to_3 - elements_from_3 + 1);
                  if (button_active_prev) {
                    ...
                    next
                      ...
                      .then((text) => {
                        ...
                        const array_info = str_info.split(" of ");
                        total_elements = parseInt(array_info[1].trim(), 10);
                        const array_current_page = array_info[0]
                          .trim()
                          .split("-");
                        let elements_from_4 = parseInt(
                          array_current_page[0].trim(),
                          10
                        );
                        let elements_to_4 = parseInt(
                          array_current_page[1].trim(),
                          10
                        );
                        cy.log("total_elements: " + total_elements);
                        ...
                        cy.log("button_active_next: " + button_active_next);
                        cy.log("button_active_prev: " + button_active_prev);
                        cy.expect(elements_from_4).to.be.lessThan(
                          elements_from_3
                        );
                        data_acquisition_title = cy
                          .get(".v-row")
                        ...
                        next
                          ...
                          .should(
                            "have.length",
                            elements_to_4 - elements_from_4 + 1
                          );
                        if (button_active_prev) {
                          data_acquisition_title = cy
                            .get(".v-row")
                          ...
                          next
                            ...
                            .then((text) => {
                              ...
                              const array_current_page = array_info[0]
                                .trim()
                                .split("-");
                              let elements_from_5 = parseInt(
                                array_current_page[0].trim(),
                                10
                              );
                              let elements_to_5 = parseInt(
                                array_current_page[1].trim(),
                                10
                              );
                              cy.log("total_elements: " + total_elements);
                              ...
                              cy.log(
                                "button_active_next: " + button_active_next
                              );
                              cy.log(
                                "button_active_prev: " + button_active_prev
                              );
                              ...
                              next
                                ...
                                .should(
                                  "have.length",
                                  elements_to_5 - elements_from_5 + 1
                                );
                            });
                        }
                      });
